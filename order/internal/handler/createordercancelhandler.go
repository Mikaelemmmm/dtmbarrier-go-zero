...
func CreateOrderCancelHandler(ctx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		l := logic.NewCreateOrderCancelLogic(r.Context(), ctx)
		...
		err = barrier.Call(ctx.OrderModel.GetConn(), func(db sqlx.Session) error {
			resp, err = l.CreateOrderCancel(req)
			if err != nil{
				return err
			}
			return nil
